...
class Mouse {
 public:
   ...
   */
  void move_ahead() {
    reporter.log_action_status('F', ' ', m_location, m_heading);
    sensors.set_steering_mode(STEER_NORMAL);
    // Move to the next sensing position
    motion.move(FULL_CELL, SEARCH_SPEED, SEARCH_SPEED, SEARCH_ACCELERATION);
    motion.set_position(SENSING_POSITION);
  }
  ...
  void turn_left() {
    ...
    motion.move(HALF_CELL - (FULL_CELL - SENSING_POSITION), SEARCH_SPEED, SEARCH_SPEED, SEARCH_ACCELERATION);
    motion.set_position(SENSING_POSITION);
  }
  ...
  void step_by_step(Location target) {
    maze.flood(target);
    delay(200);
    sensors.enable();
    motion.reset_drive_system();
